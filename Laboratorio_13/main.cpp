...
int main(){
	cv::Mat img = cv::imread("D:/COMPUTACION GRAFICA/OPENCV/src/Laboratorio_13/imagen1.jpg", cv::IMREAD_UNCHANGED);
	...
	cv::imshow("Imagen original", img);
	cv::imshow("Imagen rotada", imgRotated);
	cv::waitKey();
	cv::destroyAllWindows();
	...
	cv::imshow("Imagen original", img);
	cv::imshow("Imagen con notas", imgCircleText);
	cv::waitKey();
	cv::destroyAllWindows();

	// Guarden la figura con los círculos dibujados y el texto generado
	cv::imwrite("Imagen_con_notas.jpg", imgCircleText);
	...
	cv::Mat img2 = cv::imread("D:/COMPUTACION GRAFICA/OPENCV/src/Laboratorio_13/imagen2.jpg", cv::IMREAD_UNCHANGED);
	...
	cv::imshow("Imagen con pentagonos", img2Pentagon);
	cv::waitKey();
	cv::destroyAllWindows();
	cv::imwrite("Imagen_con_pentagonos.jpg", img2Pentagon);
	...
	while (k != 'q') {
		...
		k = cv::waitKey();
		switch (k) {
		case 'w': imgClear.copyTo(imgDrawable); break;
